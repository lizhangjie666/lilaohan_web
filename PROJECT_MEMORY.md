...
- `/menu`：窑烤菜单与分类筛选
- `/menu/[slug]`：产品详情
- `/diy`：主推“78元/人窑烤面包DIY”的完整活动宣传页，并在页尾保留其他已发布手作体验
- `/diy/[slug]`：分步骤手作教程
- `/guide`：镇山村半日打卡指南
...
- `DIYTutorial`：类型、时长、人数、配料与工具图文项、图文步骤、注意事项、演示视频链接、可选封面和SEO
- `DIYSetting`：面包DIY价格、价格单位、面团与成品数量说明、预约赠品、主推教程和SEO；全页价格只读取这一份数据
...
- 手作页的页头、准备说明、理念、四阶段故事、镇山村画廊、顾客作品墙和预约区统一在“页面板块”维护；价格、赠品和主推教程在“手作体验设置”维护
...
- 手机访问 `/diy` 时使用“价格＋立即预约”固定栏，取代全站默认的地图、电话、微信三按钮栏
...
- 手作体验页缺少真实图片时，使用项目内置并明确标注为“AI 示意图”的临时图片。
- 后台上传步骤、配料或工具的真实图片后，前端自动优先显示后台图片，不再显示对应示意图和标记。
- 示意图只用于填补内容空缺，不应描述为门店真实制作过程或真实产品。
