...
async function submit() {
  ...
  errorMessage.value = ''
  order.value = null
  const serial = serialNumber.value.replace(/\D/g, '')
  const last4 = phoneLast4.value.replace(/\D/g, '').slice(-4)
  if (!serial || last4.length !== 4) {
    errorMessage.value = '请输入唯一序号和手机号码后四位。'
    return
  }
  loading.value = true
  try { order.value = await lookupOrder(serial, last4) }
...
useSeoMeta({
  ...
  description: '输入门店提供的唯一序号和手机后四位，查询镇山村窑烤面包的制作与出炉进度。',
  ...
  <main class="min-h-[75vh] bg-flour py-10 sm:py-16">
    <div class="page-wrap grid gap-8 lg:grid-cols-[.82fr_1.18fr] lg:items-start">
      <section class="overflow-hidden rounded-[2rem] bg-ink p-7 text-flour shadow-[0_24px_70px_rgba(33,28,25,.16)] sm:p-10" v-reveal>
        ...
        <p class="mt-6 max-w-xl leading-8 text-flour/70">店员接收面包后会提供一个唯一序号。输入序号和手机号后四位，即可查看还有多久出炉。</p>
        <form class="mt-8 grid gap-5" @submit.prevent="submit">
          <label class="grid gap-2 font-semibold">唯一序号
            <input v-model="serialNumber" inputmode="numeric" autocomplete="off" placeholder="例如：0001" class="min-h-14 rounded-2xl border border-white/20 bg-white/10 px-4 text-lg text-white outline-none placeholder:text-white/35 focus:border-fire focus:ring-2 focus:ring-fire/25">
          </label>
        ...
        <p class="mt-6 text-xs leading-6 text-flour/45">为保护隐私，页面不会公开展示其他顾客信息，也不会在网址中记录你的手机号。</p>
      </section>
      ...
      <section v-else class="paper-card grid min-h-[28rem] place-items-center p-8 text-center" v-reveal="100">
        <div><span class="text-6xl" aria-hidden="true">🔥</span><h2 class="mt-5 font-serif text-3xl font-semibold">输入取件序号后查看</h2><p class="mx-auto mt-4 max-w-md leading-7 text-charcoal">进度条会随时间向前移动，到点后自动提示“可以取啦”。实际领取时请向店员出示序号。</p></div>
      </section>
    </div>
  </main>
</template>
