...
<template>
  <div>
    ...
    <section class="page-wrap grid items-center gap-10 py-20 md:grid-cols-2 md:py-28"><div class="aspect-[4/3] overflow-hidden rounded-[2rem]"><ContentImage :image="blocks.team.image || content?.teamImage" :alt="blocks.team.image ? blocks.team.imageAlt : content?.teamImageAlt" sizes="(min-width: 768px) 50vw, 100vw" class="image-cover" /></div><div><p class="eyebrow">{{ blocks.team.eyebrow }}</p><h2 class="section-title mt-5 whitespace-pre-line">{{ blocks.team.title }}</h2><p class="body-copy mt-6 whitespace-pre-line">{{ blocks.team.description || content?.teamIntro }}</p></div></section>
    <section v-if="content?.gallery.length" class="page-wrap pb-20 md:pb-28"><div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><ContentImage v-for="(photo, index) in content.gallery" :key="index" :image="photo.image" :alt="photo.alt" sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" class="aspect-[4/3] w-full rounded-[1.5rem] object-cover" /></div></section>
    <ContactPanel />
  </div>
</template>
