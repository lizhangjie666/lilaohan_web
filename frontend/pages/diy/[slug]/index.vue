...
useSeoMeta({
  title: () => isBreadDiy.value ? `窑烤面包DIY体验｜${tutorial.value?.detailPrice || 78}元/人｜贵阳花溪镇山村` : `${tutorial.value?.title || '手作体验'}｜李老汉窑烤面包`,
  description: () => isBreadDiy.value ? `贵阳花溪镇山村窑烤面包DIY体验，${tutorial.value?.detailPrice || 78}元/人，每人一份面团，可自由制作3–6个面包，提供配料与工具。` : tutorial.value?.summary,
})
</script>
  ...
  <div v-if="tutorial" class="bg-[#fff8e9] pb-24 text-[#37271f] md:pb-0">
    <section class="relative isolate min-h-[70svh] overflow-hidden bg-ink text-white">
      ...
      <div class="page-wrap relative flex min-h-[70svh] items-end py-14 md:items-center md:py-24"><div class="max-w-3xl" v-reveal>
        ...
        <div v-if="isBreadDiy && tutorial.detailPrice != null" class="mt-7 flex items-end gap-3 text-[#ff8b5f]"><strong class="font-serif text-7xl leading-none">{{ tutorial.detailPrice }}</strong><span class="pb-2 text-lg font-semibold">{{ tutorial.detailPriceUnit || '元 / 人' }}</span></div>
