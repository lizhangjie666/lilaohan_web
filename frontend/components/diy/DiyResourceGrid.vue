  ...
  <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
    <article v-for="(item, index) in (items.length ? items : (emptyNames || []).map(name => ({ name, imageAlt: `${name}图片` })))" :key="`${item.name}-${index}`" v-reveal="index * 60" class="motion-reveal overflow-hidden rounded-[1.5rem] border border-ink/10 bg-white">
      <div class="relative aspect-square overflow-hidden bg-[#ead9b8]">
        <ZoomableContentImage v-if="item.image" :image="item.image" :alt="item.imageAlt || item.name" sizes="(min-width:1024px) 25vw, 50vw" image-class="aspect-square w-full object-contain" />
        ...
        <div v-if="!item.image" class="grid h-full place-items-center bg-[radial-gradient(circle_at_50%_30%,rgba(255,255,255,.8),transparent_28%),linear-gradient(155deg,#f3e5c9,#d7bd8d)]"><span class="text-xs tracking-[.16em] text-ink/45">真实照片待上传</span></div>
      </div>
