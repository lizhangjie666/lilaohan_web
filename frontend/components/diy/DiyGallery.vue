...
<template>
  <div class="grid grid-cols-2 gap-3 md:grid-cols-3">
    <article v-for="(item, index) in items" :key="`${item.title}-${index}`" v-reveal="index * 65" class="motion-reveal group relative min-h-52 overflow-hidden rounded-[1.75rem] bg-[#d9c49f] md:min-h-72" :class="index % 3 === 0 ? 'md:row-span-2 md:min-h-[37rem]' : ''">
      <ZoomableContentImage v-if="item.image" :image="item.image" :alt="item.imageAlt || item.title" sizes="(min-width:768px) 34vw, 50vw" image-class="h-full w-full object-contain" class="absolute inset-0" />
      ...
      <div class="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/75 to-transparent p-5 pt-14 text-white"><h3 class="font-serif text-lg font-semibold md:text-2xl">{{ item.title }}</h3><p v-if="item.text" class="mt-2 text-sm leading-6 text-white/75">{{ item.text }}</p></div>
    </article>
  </div>
</template>
